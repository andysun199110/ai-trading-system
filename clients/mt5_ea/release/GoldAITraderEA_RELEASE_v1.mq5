...
//+------------------------------------------------------------------+
bool SendHttpRequest(const string method,
                     const string url,
                     ...
                     string &response)
{
   ...
   if(StringLen(body) > 0)
      StringToCharArray(body, request_data, 0, StringLen(body));
   else
      ArrayResize(request_data, 0);

   ResetLastError();
   http_code = WebRequest(method, url, headers, timeout_ms, request_data, response_data, response_headers);
   if(http_code == -1)
   {
      response = "";
      return false;
...
//+------------------------------------------------------------------+
//| Helper: append processed signal row                              |
//+------------------------------------------------------------------+
bool AppendProcessedSignal(const string signalId, const string action, const string result, const double lot_size)
{
   int handle = FileOpen(g_ProcessedSignalsFile, FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI);
   if(handle == INVALID_HANDLE)
   {
      handle = FileOpen(g_ProcessedSignalsFile, FILE_WRITE|FILE_CSV|FILE_ANSI);
      if(handle == INVALID_HANDLE)
         return false;
      FileWrite(handle, "signal_id", "timestamp", "action", "result", "lot_size");
   }

   FileSeek(handle, 0, SEEK_END);
   FileWrite(handle, signalId, TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS), action, result, DoubleToString(lot_size, 2));
   FileClose(handle);
   return true;
}
...
bool Authenticate()
{
   ...
   string json = StringFormat("{\"license_key\":\"%s\",\"account_login\":\"%I64d\",\"account_server\":\"%s\"}", 
                              Inp_LicenseKey,
   ...
   string response = "";
   int http_code = -1;
   bool ok = SendHttpRequest("POST", url, headers, json, 5000, http_code, response);

   g_LastHttpCode = http_code;

   if(!ok)
   {
      g_LastHttpError = "HTTP transport error: " + IntegerToString(GetLastError());
   ...
   {
      g_LastHttpError = "HTTP " + IntegerToString(http_code);
      Print("[GoldAI] Auth failed: ", response);
      g_IsConnected = false;
      return false;
   }

   int startPos = StringFind(response, "\"session_token\":\"");
   int token_key_len = 17;
   if(startPos < 0)
   {
      startPos = StringFind(response, "\"token\":\"");
      token_key_len = 9;
   }

   if(startPos >= 0)
   {
      startPos += token_key_len;
   ...
   g_LastHttpError = "Invalid auth response";
   Print("[GoldAI] Auth failed: ", response);
   g_IsConnected = false;
   return false;
}
...
void PollSignals()
{
   ...
   string headers = "Authorization: Bearer " + g_SessionToken + "\r\n";
   string response = "";
   int http_code = -1;
   bool ok = SendHttpRequest("GET", url, headers, "", 5000, http_code, response);

   g_LastHttpCode = http_code;

   if(!ok || http_code < 200 || http_code >= 300)
   {
      g_LastHttpError = "Poll failed";
      g_IsConnected = false;
      Print("[GoldAI] Poll failed");
      return;
   }
...
bool IsDuplicateSignal(string signalId)
{
   ...
   int handle = FileOpen(g_ProcessedSignalsFile, FILE_READ|FILE_CSV|FILE_ANSI);
...
{
   double lot_size = (g_PositionLots > 0.0) ? g_PositionLots : Inp_DefaultLotSize;
   AppendProcessedSignal(signalId, action, result, lot_size);
}
...
void ReportExecution(string signalId, string status)
{
   ...
   string response = "";
   int http_code = -1;
   SendHttpRequest("POST", url, headers, json, 5000, http_code, response);
}
